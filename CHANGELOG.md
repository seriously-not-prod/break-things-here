...
- **Story #960 — Add AI Frontend Unit Tests and Contract Tests**: Added `frontend/test/ai-contract.test.ts` (28 tests) — pure unit-level contract tests validating AI API payload shapes and service function behaviour: `fetchBudgetInsight` request/response contract via the `api` client (calls `POST /api/ai/budget-insight` with `{eventId, prompt?}`, propagates `ApiError`); `fetchVendorRecommendation` contract via raw `fetch` (URL, method, credentials=include, Content-Type header, error mapping); `ApiError` class shape (status, name, code fields, instanceof chain); and static payload-shape assertions for all five AI endpoints (`suggest`, `grounded`, `task-breakdown`, `budget-insight`, `vendor-recommendation`). Added `frontend/test/vendor-ai-recommendation-panel.test.tsx` (26 tests) — component tests for `VendorAiRecommendationPanel` covering: initial render (heading, advisory alert, prompt input, Get Recommendations button), loading state (Analysing… text, button/input disabled), error state (error text shown, button re-enabled, previous result cleared), happy path (vendor names, score chips 88/100 and 72/100, rank chips #1/#2, summary text, strength/concern chips, rationale, advisory label always visible, fetch called with correct payload), empty recommendations (warning alert), and accessibility (progress bar `aria-label="Advisory score: 88 out of 100"`). Extended `frontend/test/ai-assistant.test.tsx` with 15 new tests covering: accessible tab-panel structure for breakdown and budget panels (id/aria-labelledby pairs); Grounded Workflow `task` workflow type (fireEvent on `.MuiSelect-nativeInput`, verifies rendered output and API payload); Task Breakdown happy path (task cards with "1. Set up sound system" prefix, request payload `{eventId: 3}`, empty-task fallback message, pre-run hint, Generate button disabled without event); Budget Insight happy path (Risk: MEDIUM chip, summary text, recommendation category with `→ action` text, request payload `{eventId: 8}`, pre-run hint, Analyse button disabled without event); context selector payload (`context: 'rsvp'` forwarded to `POST /api/ai/suggest`). Total new tests: 69 across three files; all 389 frontend tests pass locally (#960).

- **Story #959 — Improve AI Assistant UX for Loading and Error States**: Enhanced `frontend/src/components/ai/ai-assistant.tsx` with four acceptance-criteria improvements. (1) **Actionable onboarding empty state**: the Chat tab now displays a "Quick start:" row of four clickable `Chip` prompts ("Event venue ideas", "Task timeline tips", "RSVP strategy", "Budget planning tips") when no messages exist; clicking a chip populates the prompt input, reducing time-to-first-query. (2) **Visible non-blocking loading state**: the messages log area carries `aria-busy={loading}` so screen readers announce the pending state without hiding the input; the "Thinking…" spinner remains inside the messages area so the prompt/send controls stay fully interactive. (3) **Clear AI error states with retry support**: chat errors are now surfaced as a named MUI `Alert` (role="alert") with an `IconButton` (`aria-label="Retry last message"`) that re-sends the last failed prompt via a dedicated `retryChatMessage()` function backed by a shared `dispatchChatRequest()` helper; Grounded Workflow, Task Breakdown, and Budget Insight tabs each have a `ReplayRounded` `IconButton` (`aria-label="Retry workflow"`, `"Retry task breakdown"`, `"Retry budget insight"`) injected into the Alert's `action` prop so retry is always co-located with the error message. (4) **Accessible AI interaction controls**: all four tab panels now carry `role="tabpanel"`, `id` (`ai-panel-chat`, `ai-panel-grounded`, `ai-panel-breakdown`, `ai-panel-budget`), and `aria-labelledby` pointing to their corresponding `Tab` element (`id` attributes added: `ai-tab-chat`, `ai-tab-grounded`, `ai-tab-breakdown`, `ai-tab-budget`); all async `onClick` handlers are wrapped in `() => void fn()` to prevent unhandled-promise lint warnings; results regions carry `aria-busy` tied to their loading state. Added 25 new tests in `frontend/test/ai-assistant.test.tsx` covering: quick-start chip rendering, chip-to-input fill, chip hide after send, non-blocking loading (input still in DOM, `aria-busy="true"`), chat error Alert presence, chat retry button, retry re-send success, error clear on next send, and retry buttons in Grounded/Breakdown/Budget tabs, plus accessible tab-panel structure (#959).

- **Story #962 — Define AI Performance and Reliability SLOs**: Added `backend/src/lib/ai-slo.ts` — the single source of truth for all AI Service Level Objective constants, including latency targets (`AI_SLO_LATENCY_P50_MS` = 1 500 ms, `AI_SLO_LATENCY_P90_MS` = 3 000 ms, `AI_SLO_LATENCY_P99_MS` = 8 000 ms), success-rate SLO (`AI_SLO_SUCCESS_RATE_TARGET` = 95 %, degraded threshold 90 %), availability target (99.9 %), request timeout (`AI_REQUEST_TIMEOUT_MS` = 30 000 ms), retry policy (`AI_RETRY_MAX_ATTEMPTS` = 2, exponential back-off 500 ms–5 000 ms), rate-limit cool-down (`AI_RATE_LIMIT_BACKOFF_MS` = 60 000 ms), and alert thresholds (rate-limit ratio > 10 %, timeout ratio > 5 %). Provides `evaluateAiSlos()` — a structured SLO compliance checker returning `SloEvaluationResult` with per-dimension `SloCheckResult` items for success-rate, rate-limit ratio, and timeout ratio — and `computeRetryBackoffMs()` for deterministic exponential back-off calculation. Updated `backend/src/lib/ai-observability.ts` to import and apply SLO constants: `computeAiHealthSignal` thresholds are now driven by `AI_SLO_SUCCESS_RATE_TARGET` (healthy ≥ 95 %) and `AI_SLO_SUCCESS_RATE_DEGRADED_THRESHOLD` (degraded ≥ 90 %, unhealthy < 90 %); added `evaluateCurrentSlos()` that calls `evaluateAiSlos` against in-memory counters for live SLO checks. Updated `GET /api/ai/health` controller to include `sloCompliance` (full `SloEvaluationResult`) in the response alongside `status` and `metrics`. Created `docs/operations/ai-slos.md` — authoritative SLO specification with latency measurement SQL queries, per-SLO alert trigger table (severity Warning/Critical), availability error-budget calculation, observability alignment table linking each `GET /api/ai/health` signal to its SLO constant, and database query reference for `ai_request_logs` percentile analysis. Created `docs/operations/ai-runbook.md` — operational runbook for AI incidents, including: a quick-reference symptom table, full procedure for each failure mode (degraded health, unhealthy health, provider not configured, latency degradation, rate limiting, timeouts, safety violations), recommended alert rules with severity/condition matrix, monitoring polling intervals, complete database query reference (failure breakdown, latency percentiles, SLO compliance snapshot, retry effectiveness), and an escalation path (L1 on-call → L2 senior/AI lead → L3 provider support). Added `backend/__tests__/ai-slo.test.ts` (18 tests) covering: `evaluateAiSlos` (all metrics within targets, success rate below target, rate-limit ratio exceeds threshold, timeout ratio exceeds threshold, zero total requests, check name coverage, ISO timestamp, field types); `computeRetryBackoffMs` (base delay, doubling, cap enforcement, never exceeds max); constant value invariants (latency ordering, fractional bounds, positive values) (#962).
...
- **Story #951 — RSVP Communication Drafting Assistance**: Added `POST /api/ai/rsvp-draft` endpoint that generates AI-drafted RSVP communication messages grounded in live attendee data fetched from the database. Returns three editable variants: `reminderVariant` (for pending/maybe guests), `confirmationVariant` (for confirmed guests), and `deadlineReminder` (urgent message for non-responders). Supports `tone` controls (formal | friendly | casual | urgent) and `length` controls (short | medium | long). RSVP context includes full status-mix breakdown (confirmed, pending, maybe, declined, waitlisted, cancelled), event date, RSVP deadline, capacity, and fill rate. User-supplied prompts are sanitised against injection attacks before inclusion. Fallback handling returns raw model output as `reminderVariant` when structured JSON parsing fails. All requests are rate-limited (20/hour) and logged to `ai_request_logs`. Added third tab "RSVP Drafts" to the frontend AI Planning Assistant with tone/length selectors, optional context field, and three in-place editable draft text areas with copy-to-clipboard buttons. Added `frontend/src/services/rsvp-communication-draft-service.ts` service module. Added `backend/__tests__/ai-rsvp-communication-draft.test.ts` with 16 tests covering all acceptance criteria, validation, 404/502/503 error paths, prompt sanitisation, and `parseRsvpDraftOutput` unit tests (#951).
