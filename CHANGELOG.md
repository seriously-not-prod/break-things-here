...
- **Story #956 — Formalize AI Safety and Prompt Injection Controls**: Added `backend/src/lib/ai-safety.ts` — a reusable, tested security module providing: (1) `sanitiseInput()` with 29 injection-detection patterns covering 7 threat categories (`prompt_injection`, `role_hijack`, `jailbreak`, `delimiter_confusion`, `context_reset`, `output_manipulation`, `html_injection`) and structured `SanitisedInput` metadata; (2) `validateAiOutput()` detecting SSN patterns, credit card numbers, credential disclosures, and auth tokens in AI responses; (3) `hardenSystemPrompt()` prepending a trust-boundary instruction to all system prompts; (4) `withProviderTimeout()` (default 30 s) preventing hung HTTPS connections from blocking Node.js workers; (5) `logAiSafetyEvent()` persisting structured `AiSafetyEvent` records to the new `ai_safety_events` table for security audit/observability. Updated `backend/src/controllers/ai-controller.ts` to use `sanitiseInput` (replaces basic 5-pattern filter), harden all system prompts, wrap all provider calls with `withProviderTimeout`, validate all AI outputs, and log safety events for injection detections, output rejections, and provider timeouts. Added `ai_safety_events` database table (v26 migration) with `event_type` CHECK constraint. Added `backend/__tests__/ai-safety.test.ts` (68 tests) covering all functions, every threat category, edge cases, and integration scenarios (#956).
