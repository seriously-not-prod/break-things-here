...
- **Task #771 — `guests` first-class table (TRD §4.2)**: Created a first-class `guests` table that stores guest identity (name, email, phone, dietary restrictions, accessibility needs) independently from RSVP responses. `rsvps.guest_id` FK now references `guests.id` (ON DELETE SET NULL — no orphaned RSVPs). The previous `CREATE OR REPLACE VIEW guests AS SELECT * FROM rsvps` shim is replaced by the real table. Migration `database/migrations/v22-guests-table-771.sql` handles DROP VIEW, CREATE TABLE, ALTER rsvps, backfill and orphan-guard verification. Runtime migration v22 in `backend/src/db/database.ts` is idempotent. RLS policy `rls_guests_event_member` limits access to event owners and event members (fail-open for jobs/tests). New controller `backend/src/controllers/guests-controller.ts` exposes full CRUD: `GET /api/events/:eventId/guest-records`, `GET /api/events/:eventId/guest-records/:id`, `POST /api/events/:eventId/guest-records`, `PUT /api/events/:eventId/guest-records/:id`, `DELETE /api/events/:eventId/guest-records/:id`. Architecture decision documented in `docs/architecture/guests-table-decision.md`. Integration tests in `backend/__tests__/guests.test.ts` cover schema migration, CRUD operations, duplicate detection, and the no-orphan-RSVP guarantee (#771).
