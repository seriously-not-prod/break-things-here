...
export async function updateMe(req: AuthRequest, res: Response): Promise<Response> {
  try {
    ...
    if (displayName === undefined && email === undefined) {
      return res
        .status(400)
        .json({ error: 'Provide at least one field to update: displayName or email' });
    }
    ...
    if (email !== undefined) {
      const currentUser = await db.get<{ email: string }>('SELECT email FROM users WHERE id = $1', [
        req.user.id,
      ]);
      if (currentUser && email !== currentUser.email) {
        return res.status(400).json({
          error:
            'To change your email address use the email change endpoint: POST /api/profile/change-email',
        });
      }
    }
