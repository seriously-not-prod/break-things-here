...
export async function createUser(req: AuthRequest, res: Response): Promise<Response> {
  ...
  const verifiedValue = email_verified ? 1 : 0;
  ...
  const result = await db.run(
    ...
    [normalizedEmail, passwordHash, display_name.trim(), verifiedValue, role_id],
  );
...
export async function updateUser(req: AuthRequest, res: Response): Promise<Response> {
  ...
  const updateValues: unknown[] = [normalizedEmail, display_name.trim(), role_id, email_verified ? 1 : 0, account_locked ? 1 : 0, id];
  let query = `UPDATE users SET email = ?, display_name = ?, role_id = ?, email_verified = ?, account_locked = ?, updated_at = CURRENT_TIMESTAMP`;

  if (password) {
    const passwordHash = await hashPassword(password);
    query += ', password_hash = ?';
    updateValues.splice(updateValues.length - 1, 0, passwordHash);
  }
