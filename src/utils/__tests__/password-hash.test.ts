...
import { hashPassword, verifyPassword, getSaltRounds, PasswordHashError } from '../password-hash';
...
describe('Password Hashing Utility', () => {
  ...
  describe('verifyPassword', () => {
    ...
    it('should throw error for non-string inputs', async () => {
      ...
      await expect(verifyPassword(null as unknown as string, hashed)).rejects.toThrow(
        PasswordHashError,
      );
      await expect(verifyPassword('test', null as unknown as string)).rejects.toThrow(
        PasswordHashError,
      );
    });
  ...
  describe('Security: Plain-text Password Never Stored', () => {
    it('should never return or log plain-text password in errors', async () => {
      ...
      const spy = vi
        .spyOn(bcrypt, 'hash')
        .mockRejectedValueOnce(new Error('bcrypt internal failure') as never);
