...
describe('POST /api/auth/register', () => {
  ...
  it('should trim whitespace from name on registration', async () => {
    const res = await request(buildApp()).post('/api/auth/register').send({
      name: '  Padded Name  ',
      email: 'trim@test.com',
      password: 'pass12345passed',
    ...
    expect(res.status).toBe(201);
    const stored = await inMemoryUserStore.findByEmail('trim@test.com');
    expect(stored?.name).toBe('Padded Name');
  });
});
